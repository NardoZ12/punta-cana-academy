...
export default function TakeExamPage() {
  ...
  // En 'use client', params ya viene resuelto por el hook, no necesitamos await
  ...
  useEffect(() => {
    async function init() {
      ...
      if (submission) {
        ...
        // Cargar info básica solo para mostrar título
        const { data: e } = await supabase.from('exams').select('title, courses(title)').eq('id', examId).single();
        setExam(e);
        setLoading(false);
        return;
      }
      ...
      const { data: qData, error } = await supabase
        .from('exam_questions')
        .select('*')
        .eq('exam_id', examId); // IMPORTANTE: RLS debe permitir esto
        ...
        <div className="space-y-8">
           {questions.length === 0 ? (
             <div className="text-center py-10 text-gray-500 border border-dashed border-gray-700 rounded-xl">
               <p>No se encontraron preguntas cargadas para este examen.</p>
               <p className="text-xs mt-2">Contacta a tu profesor.</p>
             </div>
           ) : (
             questions.map((q, idx) => (
               <div key={q.id} className="bg-gray-800 border border-gray-700 rounded-xl p-6">
                  <h3 className="text-lg font-medium mb-4 flex gap-3 text-white">
                     <span className="bg-gray-700 text-gray-300 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0">
                       {idx + 1}
                     </span>
                     {q.question_text}
                  </h3>
                       ...
                       <div 
                         ...
                         className={`
                           ...
                           ${answers[q.id] === opt 
                             ...
                             : 'bg-black/20 border-gray-600 hover:bg-gray-700 text-gray-300'}
                         `}
                       >
        ...
        {questions.length > 0 && (
          <div className="mt-10 flex justify-end gap-4 pt-6 border-t border-gray-800">
             ...
             <Button onClick={handleSubmit} disabled={submitting}>
                {submitting ? 'Enviando...' : `Finalizar Examen (${Object.keys(answers).length}/${questions.length})`}
             </Button>
          </div>
        )}
